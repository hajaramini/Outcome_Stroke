...
x<- cbind.data.frame(Xu_HumanStrokeSampleMedicalInfo, Demographic_Parameter) #50 316
...
rownames(Demographic_CLEAR) <- Demographic_CLEAR$Bar_Code
x <- rownames(Demographic_CLEAR)
y <- as.integer(x)

include <- Demographic_Parameter_low$X206SS_Bar_Code
Demographic_CLEAR_low <- Demographic_CLEAR[x%in%include,] #36 202

#merge two demographic files including duplication for most of the parameters
Demographic_all <-cbind.data.frame(Demographic_Parameter_low,Demographic_CLEAR_low) #36 443
rownames(Demographic_all) <- Demographic_all$Filename
```

```{r}
#save
write.csv(Demographic_all,file = "../../Input_Output/36_Demographic_Parameter_All_03-16-20.csv")
```
...
DS_Demographic_all <- Demographic_all[rownames(Demographic_all)%in%DS_Filename_v2,] #25 443
VS_Demographic_all <- Demographic_all[rownames(Demographic_all)%in%VS_Filename_v2,] #11 443
```
